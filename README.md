<img width="720" height="480" alt="image" src="https://github.com/user-attachments/assets/0b989d6b-fe44-467a-bb21-00a3672eb288" /># Anegon Waste-to-Energy System

Anegon GreenTech Ltd is developing a decentralized waste-to-energy solution that converts organic waste into clean biogas and organic fertilizer to improve environmental conditions and children’s health in underserved communities.


##  Problem

Children in many communities are exposed to:

- Indoor air pollution from firewood and charcoal
- Poor sanitation due to unmanaged organic waste
- Food spoilage affecting nutrition

These conditions increase the risk of respiratory illness, infections, and poor health outcomes.


##  Solution

Our system converts organic waste (especially poultry waste) into:

- Clean cooking gas (biogas)
- Energy for productive use
- Organic fertilizer

This reduces pollution, improves sanitation, and creates healthier living environments.


## How It Works

1. Organic waste is collected
2. Waste is fed into a biodigester
3. Biogas is produced and stored
4. Gas is used for cooking or energy
5. Residual slurry is used as fertilizer


## Current Status

- Pilot stage with real installations completed
- Biogas systems deployed in poultry farm environments
- Demonstration center under development

### Demo Center Includes:

- 1000-capacity poultry production unit
- Staff quarters
- Trainee accommodation
- Borehole water system
- Planned biodigester, gas storage, and power systems


##  Impact

- Reduced indoor air pollution
- Improved sanitation
- Cleaner environment for children
- Sustainable energy access


##  Project Videos

Demo Center Progress:  
(https://youtu.be/08-sFBxfilk?si=1cY476-y49dYK7NE)

Field Installation:  



## Project Images:
Uploading image.png…



## Future Development

- Deployment to 5–15 user clusters
- Training of 20–60 technicians
- Cold storage integration for food safety
- Expansion into commercial methane distribution stations


##  Organization

**Anegon GreenTech Ltd**  
Nigeria  


##  Contact

Email: anegonreentech@gmail.com


## Note

This repository documents a hardware-based climate-health solution designed to reduce environmental risk exposure and improve living conditions for children and vulnerable communities.
